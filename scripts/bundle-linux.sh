...
# Fork change: brand icon. The installed filename and the .desktop `Icon=` and
# `StartupWMClass=` keys stay as upstream's `sh.waku` — they have to match the
# WM class GPUI sets at runtime, so renaming them would silently break taskbar
# grouping on a platform we cannot test here.
