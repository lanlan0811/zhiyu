...
impl CodexDriver {
    pub fn start(options: DriverStartOptions, events: DriverEventSender) -> anyhow::Result<Self> {
        ...
        let mut command = crate::command_env::command_for_provider(&binary, "codex");
        // Fork change: older Codex releases reject `--stdio` (stdio was the
        // implicit default before the `--listen` selector existed). Ask the
        // binary what it takes instead of assuming a version.
        command.args(sub2api::codex_compat::app_server_args(&binary));
        command.args(sub2api::codex_compat::session_config_args());
...
fn generate_codex_title(binary: &Path, cwd: &Path, prompt: &str) -> anyhow::Result<String> {
    // Title generation runs a real turn, so it routes through the gateway too;
    // otherwise it would silently bill the user's own vendor account.
    let mut command = crate::command_env::command_for_provider(binary, "codex");
    let command = command
        .args(sub2api::codex_compat::app_server_args(binary))
        .args(sub2api::codex_compat::session_config_args())
        .current_dir(cwd)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::null());
