...
fn daemon_executable_path() -> anyhow::Result<PathBuf> {
    ...
    let sibling = current
        ...
        .ok_or_else(|| anyhow!("the app executable has no parent directory"))?;
    if sibling.is_file() {
        return Ok(sibling);
    }
    #[cfg(debug_assertions)]
    bail!(
        "the daemon was not found in Cargo's debug directory or next to the app executable: {}",
        sibling.display(),
    );
    #[cfg(not(debug_assertions))]
    bail!(
        "the daemon is missing next to the app executable: {}",
        sibling.display(),
    )
}
