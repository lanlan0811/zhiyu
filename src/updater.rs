...
mod windows {
    ...
    ///
    /// Fork change: the host comes from `sub2api::brand` so a rebranded build
    /// updates from its own release channel rather than upstream's.
    #[cfg(target_arch = "aarch64")]
    const FEED_URL: &str = concat!(
        env!("SUB2API_RELEASES_BASE_URL_RESOLVED"),
